...
# Hard-sphere gas code based on work of Bruce Sherwood
...
Ratom = 0.01 # wildly exaggerated size of helium atom, valor inicial 0.03
...
def Tempo(b):
    ...
    toc = time.perf_counter()
    timeout = toc - tic
    output.text = "Time since the beggining of "+fancy_t(timeout)
    #animation.capture(f"scrshot_t{timeout:0.4f}s")
...
histo = []
for i in range(nhisto): histo.append(0.0) #canviar aquesta part si es canvia les distribucions inicials
...
timeout = toc - tic
output.text = "Time since the beggining of "+fancy_t(timeout)
animation.capture("scrshot_final_t"+fancy_t(timeout))        
